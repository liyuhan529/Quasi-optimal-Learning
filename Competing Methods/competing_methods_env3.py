...
def traj_test1(sac,stage_num,seed,sigma1):
    ...
    for i in range(stage_num):
        ...
        #print(s_new)
        R[i] =  -np.exp(s_new[0]/2+s_new[4]/2)*(action**2)+2*(s_new[1]+s_new[2]+s_new[5]+s_new[6]+0.5)*action+s_new[3]+s_new[7]
...
#     %R seed=seed+1 
    ...
    d3rlpy.seed(i)
    sac = d3rlpy.algos.DDPG(scaler="standard",action_scaler="min_max",batch_size=64, use_gpu=False,actor_learning_rate=0.001, critic_learning_rate=0.001)
    sac.fit(dataset,n_epochs=100,save_metrics=False,show_progress=False,with_timestamp=False)
...
#     %R seed=seed+1 
    ...
    dataset = MDPDataset(observations, actions, rewards, terminals)
    d3rlpy.seed(i)
    sac = d3rlpy.algos.BEAR(scaler="standard",action_scaler="min_max",batch_size=64, use_gpu=False,actor_learning_rate=0.0001, critic_learning_rate=0.0001,temp_learning_rate=0.0001)
    sac.fit(dataset,n_epochs=60,save_metrics=False,show_progress=False,with_timestamp=False)
    result_tmp=np.zeros(100)
    for j in range(100):
        result_tmp[j] =traj_test1(sac,100,seed=j,sigma1=sigma1)
    result[i] = np.mean(result_tmp)
