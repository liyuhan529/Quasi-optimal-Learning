# data generator for environment II
# non quadratic form

state_generate2=function(A,S1,S2){
	#	A_tmp=(A+100)/200
...
data_generation2=function(stage,n,dims=2,seed){
	set.seed(seed)
	...
	for(i in (n+1):(n*stage)){
		state[i,]=state_generate2(A[i-n],state[i-n,1],state[i-n,2])
	}
...
traj2=function(s1,theta1,theta2,theta3,stage_num,max_s,min_s,max_a,min_a,lambda){
	...
	for(i in 1:(stage_num-1)){
		...
		S[i+1,]=state_generate2(A[i],S[i,1],S[i,2])
...
# repeat 50 times
simu_env2=function(simu_run,stage,N){
ss=rep(0,100)
learn_r=c()
for(i in 1:simu_run){
	#set.seed(i)
