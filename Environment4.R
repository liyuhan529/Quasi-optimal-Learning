library(mvtnorm)
library(randcorr)
state_generate4=function(s_mean,a){
	# s,s_mean 8-dims
	# a scaler
	s_mean_new=rep(0,8)
	for (i in 1:4){
...
data_generate4=function(stage,n,dims=8,seed,sigma){
	set.seed(seed)
	...
	for(i in (n+1):(n*stage)){
		S_mean[i,]=state_generate4(S_mean[i-n,],A[i-n])
		S[i,]=rmvnorm(1,mean=S_mean[i,],sigma=sigma)
	}
...
pi_policy_tmp=function(s,a,lambda,theta1,theta2,theta3,c1){
	...
	#	}
	#	else{
	#		pi_a = Q_func_tmp(s=s,a=a,theta1=theta1,theta2=theta2,theta3=theta3)/(2*lambda)- 
	#			Q_int_tmp(s=s,theta1=theta1,theta2=theta2,theta3=theta3,lambda=lambda,c1)/(2*lambda*K1)-K2/(2*c1*lambda*K1)+1/K1
	#	}
	#}
	return(pi_a)
}
...
traj4=function(state_dim,theta1,theta2,theta3,stage_num,max_s,min_s,max_a,min_a,lambda,sigma){
	...
	for(i in 1:(stage_num-1)){
		...
		# generate a based on q-gaussian
		#u1=runif(1)
		#u2=runif(1)
		#a_tmp=sqrt(-3*(u1^(2/3)-1))*cos(2*pi*u2)
		S_mean[i+1,]=state_generate4(S_mean[i,],A[i])
		S[i+1,] = rmvnorm(1,mean=S_mean[i,],sigma=sigma)
		
		R[i]=((S[1+i,1]/2)^3+(S[1+i,2]/2)^3)+2*((S[1+i,5]/2)^3+(S[1+i,6]/2)^3)+
			S[1+i,3]+S[1+i,4]+0.5*(S[1+i,7]+S[1+i,8])
	}
	return(list(S=S,A=A,R=R))
}

# repeat 50 times
simu_env4=function(simu_run,stage,N){
ss=rep(0,100)
r_rec=c()
#real_r=c()
learn_r=c()
